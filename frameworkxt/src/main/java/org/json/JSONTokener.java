...
/**
 * Parses a JSON (<a href="http://www.ietf.org/rfc/rfc4627.txt">RFC 4627</a>) encoded string into the corresponding object. Most clients of this class will use only need the
 ...
 * 
 * <p>
 * For best interoperability and performance use JSON that complies with RFC 4627, such as that generated by {@link JSONStringer}. For legacy reasons this parser is lenient, so a
 * successful parse does not indicate that the input string was valid JSON. All of the following syntax errors will be ignored:
 * <ul>
 ...
 * 
 * <p>
 * Each tokener may be used to parse a single JSON string. Instances of this class are not thread safe. Although this class is nonfinal, it was not designed for inheritance and
 * should not be subclassed. In particular, self-use by overrideable methods is not specified. See <i>Effective Java</i> Item 17,
...
public class JSONTokener {
	...
	private int nextCleanInternal() throws JSONException {
		while (pos < in.length()) {
			...
			case '#':
				/*
				 * Skip a # hash end-of-line comment. The JSON RFC doesn't specify this behavior, but it's required to parse existing documents. See http://b/2571423.
				 */
				skipToEndOfLine();
				continue;
	...
	/**
	 * Returns the string up to but not including {@code quote}, unescaping any character escape sequences encountered along the way. The opening quote should have already been
	 * read. This consumes the closing quote, but does not include it in the returned string.
	 * 
	 * @param quote
	 *            either ' or ".
	...
	public String nextString(char quote) throws JSONException {
		/*
		 * For strings that are free of escape sequences, we can just extract the result as a substring of the input. But if we encounter an escape sequence, we need to use a
	...
	/**
	 * Unescapes the character identified by the character or characters that immediately follow a backslash. The backslash '\' should have already been read. This supports both
	 * unicode escapes "u000A" and two-character escapes "\n".
	 * 
	 * @throws NumberFormatException
	 *             if any unicode escape sequences are malformed.
	 */
	...
	private Object readLiteral() throws JSONException {
		...
		if (literal.indexOf('.') == -1) {
			...
			try {
				...
				/*
				 * This only happens for integral numbers greater than Long.MAX_VALUE, numbers in exponential form (5e-10) and unquoted strings. Fall through to try floating point.
				 */
			}
		}
	...
	private JSONObject readObject() throws JSONException {
		...
		while (true) {
			...
			/*
			 * Expect the name/value separator to be either a colon ':', an equals sign '=', or an arrow "=>". The last two are bogus but we include them because that's what the
	...
	/**
	 * Reads a sequence of values and the trailing closing brace ']' of an array. The opening brace '[' should have already been read. Note that "[]" yields an empty array, but
	 * "[,]" returns a two-element array equivalent to "[null,null]".
	 */
	...
	/*
	 * Legacy APIs. None of the methods below are on the critical path of parsing JSON documents. They exist only because they were exposed by the original implementation and may
	 * be used by some clients.
	 */
	...
	/**
	 * Returns the next available character, or the null character '\0' if all input has been exhausted. The return value of this method is ambiguous for JSON strings that contain
	 * the character '\0'.
	 */
	public char next() {
	...
	/**
	 * Returns the next character that is not whitespace and does not belong to a comment. If the input is exhausted before such a character can be found, the null character '\0'
	 * is returned. The return value of this method is ambiguous for JSON strings that contain the character '\0'.
	 */
