...
public class CordovaLocationListener implements LocationListener {
    ...
    protected void fail(int code, String message) {
        for (String callbackContext: this.callbacks)
        {
        ...
        Iterator<String> it = this.watches.values().iterator();
        while (it.hasNext()) {
    ...
    public void addWatch(String timerId, String callbackContext) {
        this.watches.put(timerId, callbackContext);
        if (this.size() == 1) {
            this.start();
        }
    }
    public void addCallback(String callbackContext) {
    ...
    protected void start() {
        if (!this.running) {
            if (this.locationManager.getProvider(LocationManager.NETWORK_PROVIDER) != null) {
                ...
                this.locationManager.requestLocationUpdates(LocationManager.NETWORK_PROVIDER, 60000, 10, this, handlerThread.getLooper());
            } else {
