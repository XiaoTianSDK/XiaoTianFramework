...
public class JazzyViewPager extends ViewPager {
    ...
    public static final String[] EFFECT_TYPE = {"Standard", "Tablet", "CubeIn", "CubeOut", "FlipVertical", "FlipHorizontal", "Stack", "ZoomIn", "ZoomOut", "RotateUp", "RotateDown", "Accordion"};
    ...
    public JazzyViewPager(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            effect = ua.getAttributeValue("transformStyle");
            if (effect == null) effect = EFFECT_TYPE[0];
            fadeEnabled = ua.getBooleanAttribute("fadeEnabled");
            outlineEnabled = ua.getBooleanAttribute("outlineEnabled");
            outlineColor = ua.getColorAttribute(getResources().getString(R.string.XIAOTIAN_NS), "outlineColor", Color.WHITE);
        }
        setFadeEnabled(fadeEnabled);
        setOutlineEnabled(outlineEnabled);
        setTransitionEffect(TransitionEffect.valueOf(effect));
        switch (mEffect) {
            case Stack:
            case ZoomOut:
                setFadeEnabled(true);
