...
public class IndexScroller {
	...
	private static final int STATE_SHOWN_ALWAY = 3;
	...
	public void draw(Canvas canvas) {
		...
		if (mSections != null && mSections.length > 0) {
			...
			if (mCurrentSection >= 0) {
				...
				RectF previewRect = new RectF((mListViewWidth - previewSize) / 2, (mListViewHeight - previewSize) / 2, (mListViewWidth - previewSize) / 2 + previewSize,
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (mState == STATE_SHOWN && !isAlwayShowIndexScroller) setState(STATE_HIDING);
			break;
		}
		return false;
	}
	...
	private int getSectionByPoint(float y) {
		...
		if (y >= mIndexbarRect.top + mIndexbarRect.height() - mIndexbarMargin) return mSections.length - 1;
